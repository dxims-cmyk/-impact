  ...
  if (code) {
    ...
    if (!error && sessionData.user) {
      ...
      if (!existingUser) {
        ...
        // Determine role - admin for specific emails, owner for new org creators
        const isAdmin = ADMIN_EMAILS.includes(user.email || '')
        const role = isAdmin ? 'owner' : 'owner' // First user of org is always owner
        ...
        const { error: userError } = await supabase
          ...
          .insert({
            ...
            is_agency_user: isAdmin, // Agency users can see all orgs
          })
