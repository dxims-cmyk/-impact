...
DROP POLICY IF EXISTS "automation_actions_via_automation" ON automation_actions;
...
DROP POLICY IF EXISTS "automation_runs_via_automation" ON automation_runs;
CREATE POLICY "automation_runs_via_automation" ON automation_runs
  FOR ALL USING (automation_id IN (SELECT id FROM automations));

-- 4. Form config on organizations
ALTER TABLE organizations ADD COLUMN IF NOT EXISTS form_config JSONB DEFAULT '{}'::jsonb;

UPDATE organizations
SET form_config = jsonb_build_object(
  'fields', jsonb_build_array(
    jsonb_build_object('id','name','label','Full Name','type','text','required',true,'enabled',true),
    jsonb_build_object('id','email','label','Email','type','email','required',true,'enabled',true),
    jsonb_build_object('id','phone','label','Phone','type','tel','required',false,'enabled',true),
    jsonb_build_object('id','company','label','Company','type','text','required',false,'enabled',false),
    jsonb_build_object('id','message','label','Message','type','textarea','required',false,'enabled',false)
  ),
  ...
  'embed', jsonb_build_object('type','inline','width','full')
)
